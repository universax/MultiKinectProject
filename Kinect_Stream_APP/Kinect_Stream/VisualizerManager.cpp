...
void VisualizerManager::updateVisualizer(pcl::PointCloud<PointType>::Ptr inputCloud)
{
	...
	*showCloud = *inputCloud;
	...
	for (int i = 0; i < 4; i++)
	{
		string idStr = "Kinect v" + to_string(i + 1);
...
void VisualizerManager::setupVisualizer(pcl::PointCloud<PointType>::Ptr inputCloud) {
	...
	viewer->addPointCloud(inputCloud, "Kinect v1", v1);
	...
	viewer->addPointCloud(inputCloud, "Kinect v2", v2);
	...
	viewer->addPointCloud(inputCloud, "Kinect v3", v3);
	...
	viewer->addPointCloud(inputCloud, "Kinect v4", v4);
...
void VisualizerManager::setDefaultViewPoints() {
	...
	viewer->setCameraPosition(0, 6.0, 0.0, 0, 0, 0, 2);	//Top
	viewer->setCameraPosition(0, 0.0, -3.0, 0, 0, 0, 3);	//Front
	viewer->setCameraPosition(-4.0, 0.0, 0.0, 0, 0, 0, 4);	//Side
}
...
void VisualizerManager::keyboardEventOccurred(const pcl::visualization::KeyboardEvent &event, void* viewer_void)
{
	...
	{
		kinectPosture.saveKinectPostureData();
		editMode = Edit_Mode_Save;
	}
	//-----Load Settings
	if (inputKey == "l" || inputKey == "L")
	{
		kinectPosture.loadKinectPostureData();
